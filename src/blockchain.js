...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                reject(Error('Block could not be created'));
            }
        });
    }
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            resolve(`${address}:${new Date().getTime().toString().slice(0, -3)}:startRegistry`);
        });
    }
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (currentTime - messageTime < 300) {
                ...
                reject(Error('message timestamp longer than 5 min'))
            }
        });
    }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
           let block = self.chain.find(item => item.hash === hash);
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            self.chain.forEach(async block => {
                const start = await block.getBData();
